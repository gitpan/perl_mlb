...
sub localize {
  ...
  Thank you.
	...
	for $funkyftp ($CPAN::Config->{'lynx'},$CPAN::Config->{'ncftp'}) {
	    next if $funkyftp eq '';
	    next unless MM->maybe_command($funkyftp);
	    my($want_compressed);
	    print(
		  qq{
...
returned status $estatus (wstat $wstatus)
	...
	if ($url =~ m|^ftp://(.*?)/(.*)/(.*)|) {
	    my($host,$dir,$getfile) = ($1,$2,$3);
	    my($netrcfile,$fh);
	    if (MM->maybe_command($CPAN::Config->{'ftp'})) {
		my $timestamp = 0;
...
Subprocess "|$CPAN::Config->{'ftp'}$verbose -n"
  returned status $estatus (wstat $wstatus)
	...
	for $ext (qw/lynx ncftp ftp/) {
	    $CPAN::Config->{$ext} ||= "";
	    push @mess, "an external $ext" unless MM->maybe_command($CPAN::Config->{$ext});
	}
	$mess = qq{Either get }.
	    join(" or ",@mess).
...
sub perl {
    ...
    $perl ||= "$CPAN::Cwd/$^X" if MM->maybe_command("$CPAN::Cwd/$^X");
    unless ($perl) {
